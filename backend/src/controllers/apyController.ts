...
import { storeHourlyAverageAPY, fetchHourlyAverageAPY, fetchHourlyAverageAPYs } from "../services/apyService";
...
export const getHourlyAverageAPYs = async (req: Request, res: Response) => {
  try {
    const hourlyAPY = await fetchHourlyAverageAPYs();
    res.json(hourlyAPY);
  } catch (error) {
    res.status(500).json({ error: "Failed to fetch hourly average APY." });
  }
